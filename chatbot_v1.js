...
async function auth(agent)
{
  const trouble_ticket = agent.parameters.issue_ticket;
  const client = new MongoClient(url);
  await client.connect();
  const snap = await client.db("chatbot").collection("user_issues").findOne({trouble_ticket: trouble_ticket});
  
  if(snap==null){
    await agent.add("Invalid Complaint ID");
...
  td=snap.time_date;
  
  await agent.add("Customer Name: "+user_name+"\n Issue: "+iss+"\n Status: "+sta+"\n Registered on: "+td);}
}
...
intentMap.set("issue_tracking", auth);
